---
title: "Sleep Well"
author: "Junhee Park, Ji Min Sung, William Yuen, and Rohan Simha"
date: "Autumn 2022"
...
#### Sourcing "Summary_Information.R" file
```{r}
source("../source/Summary_Information.R")
summary_info
```
#### Dynamic Paragraph 1
We used the “sleep data” to calculate the maximum value of sleep quality to figure out the best quality of sleep in this data. All the values we calculated were the same which is “99%”.  According to dataset, usually 99% sleep quality also have more than 8 hours 30 minutes sleep time. Based on our calculation, we assume that sleep quality is closely related to amount of sleep time. 
```{r}
source("../source/Summary_Information.R")
summary_info_2
```
...
We used the “sleepdata_2” to calculate the minimum value of steps in a day to figure out how the steps are related to the snore time. All the values we calculated were the 0 which is no steps. Based on dataset, when the steps data is 0, most of snore times were also 0. We could assume that people don’t snore when they have no steps in a day.
```{r}
source("../source/Summary_Information.R")
summary_info_3
```
...
We used the “SaYodata” to calculate the maximum value of stress level to see how the stress level is related to heart rate during the sleep time. All the values we calculated were the 4 which is the highest stress level In this dataset. According to dataset, when people have level 4 of stress level, their heart rate is also pretty high averagely. The average of heart rate when the stress level reached to 4 is 84%. Comparing to the heart rate when stress level is 0, it is dynamically high since the stress level 0 heart rate is 54% averagely. 
```{r}
source("../source/Summary_Information.R")
summary_info_4
```
...
We used the “sleep data” to calculate the maximum value of heart rate to figure out the cause of the high value of heart rate. The highest heart rate we calculated was 98 which is very not good for our health. We compared all the features we have and we found one common cause to have high heart rate. It was “drank coffee”. Based on dataset, there is one feature called, “Sleep notes”. When people drank coffee before sleep, we found the high heart rate during their sleep time. We strongly believed that drinking coffee is not good for sleep
```{r}
source("../source/Summary_Information.R")
summary_info_5
```
...
We used the “sleepdata_2” to calculate the minimum value of regularity to figure out how the regularity affect to the sleep quality. When we calculated the minimum value of the regularity, we got -1 which is very low value. It showed us that data has very unstable sleep regularity. According to dataset, we could find the relationship between the regularity and sleep quality. When regularity has -1%, the sleep quality was 49%. The dataset clearly showed us that regularity is the one of the important element of the sleep quality.
...
- _We included two bar graphs in chart_1 to show us how sleeping time affects sleep quality. The first bar graph shows the sleep quality with the top maximum sleep hours and the second bar graph shows the top minimum sleep hours. These two charts inform us that neither too much nor less sleep time is helping improve sleep quality since too much sleep time will affect sleep quality to decrease, and too less sleep time will affect sleep quality to decrease also. These two charts show that a person needs to have the right amount of sleep in order to improve sleep quality._
...
- _We included the chart to figure out the relationship between steps in a day and sleep quality. This chart shows around 20000 steps in a day has the best quality of sleep_
...
- _We included the chart to figure out the relationship between amount of limb movement and stress level. This chart clearly shows when stress level is increasing, the amount of limb movement is also increasing._

### Stress Level and Average Sleeping Hour Table
...
```
 ...
 - _We added this **Table of Stress Levels and Average Sleeping Hours** to see how sleeping time can affect stress levels. This data table informs that as the sleeping hours decrease, the stress level will increase because in the table when a person has zero amount of sleep time will eventually cause them to have level 4 of Stress which is a maximum stress level. Similarly, in the table, when a person had 8 hours of sleep, his or her stress level would be 0. This data table will be useful for us to find what the average time of sleep is best to reduce the stress level._


## **Acknowledgements**

> Thanks to the data provider, and TAs.
